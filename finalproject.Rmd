...
This dataset contains the hourly and daily count of rental bikes between years 2011 and 2012 in Capital bikeshare system with the corresponding weather and seasonal information. The data set is fairly new and small in the sense that it has only has 2 years of data. So any predictions created must be taken lighlty. The data set has 731 rows, and 16 columns total.
...
This is a plot of how many bikes were being rented during each month. Please note that for this plot we used the day data set and not the hour data set.
...
This is a boxplot of bikes that were rented on holidays and bikes that were rented on regular days.

```{r}
plot(cnt ~ temp,data=dat,col="orangered",main="Amount of bike rentals based on temperature",ylab="bikes rented",xlab="normalized temperature in celsius",pch=8)
...
bikes.myclust <- kmeans(cluster_dat,num_clust)
plotcluster(cluster_dat,bikes.myclust$cluster,pch=1)

good_or_bad <- bikes.myclust$totss / bikes.myclust$betweenss

paste0("If the bss/tss is close to a score of 1, then kmeans worked well... here is the score: ",round(good_or_bad,3))
...
Here we decided to compare the models we used to predict bike rentals. These histograms are of errors.
...
Conclusion states what you found and what you thought
